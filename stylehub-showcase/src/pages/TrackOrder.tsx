...
const TrackOrder = () => {
  ...
  const [searchedOrder, setSearchedOrder] = useState(trackingId ? getOrderByTrackingId(trackingId) : null);

  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    // Search by tracking ID first, then by order ID
    ...
    <Layout>
      <div className="container mx-auto px-4 py-16 max-w-3xl">
        ...
        <form onSubmit={handleSearch} className="flex gap-2 mb-8">
          <Input placeholder="Enter Order ID or Tracking ID" value={trackingId} onChange={(e) => setTrackingId(e.target.value)} className="flex-1" />
          <Button type="submit"><Search className="h-4 w-4 mr-2" />Track</Button>
        </form>
        ...
        {searchedOrder ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-muted-foreground">No order found with this tracking ID</p>
          </div>
        )}
      </div>
    </Layout>
